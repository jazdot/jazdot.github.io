...
// Fast, snappy page transitions (180ms)
const pageTransitionVariants = {
  initial: { opacity: 0, y: 8 },
  animate: { opacity: 1, y: 0, transition: { duration: 0.18, ease: [0.16, 1, 0.3, 1] as const } },
  exit: { opacity: 0, y: -8, transition: { duration: 0.14, ease: [0.16, 1, 0.3, 1] as const } }
};

// Extracted routes component with fast transitions
...
export default function App() {
  const [glowColor, setGlowColor] = useState('rgba(120, 119, 198, 0.12)');
  ...
  // High-performance spring tracking for cursor glow
  const springConfig = { stiffness: 250, damping: 25, mass: 0.1 };
  const springX = useSpring(mouseX, springConfig);
  const springY = useSpring(mouseY, springConfig);

  const glowX = useTransform(springX, (value) => value - 150);
  const glowY = useTransform(springY, (value) => value - 150);
  ...
  return (
    ...
    <div 
      className="relative z-0 min-h-screen overflow-x-hidden font-sans text-slate-900 dark:text-slate-100 transition-colors duration-250"
    >
      {/* Hardware-Accelerated Cursor Glow */}
      <m.div
        animate={{
      ...
      <footer style={{ position: 'relative', zIndex: 10, padding: '4rem 5vw', textAlign: 'center', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem', marginBottom: '1.5rem' }}>
          <a href="#" onClick={(e) => { e.preventDefault(); setIsContactOpen(true); }} aria-label="Email Me" style={{ color: '#888', transition: 'color 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.color = '#7877c6'} onMouseLeave={(e) => e.currentTarget.style.color = '#888'}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
          </a>
          <a href="https://linkedin.com/in/muhammedriswanmp" target="_blank" rel="noreferrer" aria-label="LinkedIn Profile" style={{ color: '#888', transition: 'color 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.color = '#7877c6'} onMouseLeave={(e) => e.currentTarget.style.color = '#888'}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path><rect x="2" y="9" width="4" height="12"></rect><circle cx="4" cy="4" r="2"></circle></svg>
          </a>
          <a href="#" onClick={(e) => { e.preventDefault(); setIsGitHubOpen(true); }} aria-label="GitHub Profile" style={{ color: '#888', transition: 'color 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.color = '#7877c6'} onMouseLeave={(e) => e.currentTarget.style.color = '#888'}>
